...
X <- cbind(as.numeric(GenIMS1[,22]),as.numeric(GenIMS1[,23]),as.numeric(GenIMS1[,24]),
	as.numeric(GenIMS1[,82]-1),as.numeric(GenIMS1[,83]),as.numeric(GenIMS1[,84]))
...
#Creating Q-Q Plots for log-transormed data considering only available cases for each covariate; note that for IL-6, only data points 
#with observations above (or equal to) 5 are considered since there are two DLs, depending on the patient
par(mfrow=c(1,3))
plot(qnorm(seq(505/1418,1,1/1418)),log(sort(X[X[,1]>=4,1])), main="TNF", xlab="Quantiles of N(0,1)", ylab="log(TNF)",cex.main = 2,cex.lab=1.5)
plot(qnorm(seq(212/1418,1,1/1418)),log(sort(X[X[,2]>=5,2])), main="IL-6", xlab="Quantiles of N(0,1)", ylab="log(IL-6)",,cex.main = 2,cex.lab=1.5)
plot(qnorm(seq(665/1418,1,1/1418)),log(sort(X[X[,3]>=5,3])), main="IL-10", xlab="Quantiles of N(0,1)", ylab="log(IL-10)",,cex.main = 2,cex.lab=1.5)
...
Ests <- optim(c(1.7,0,0,0,3.6,0,0,0,1.7,0,0,0, 1.09, 0.96, 0.56, 5.28, 1.80, 2.88),Maxing(X,mn,m1,m2,m3,m12,m13,m23,ma,dmatrix),hessian=TRUE,method="BFGS",control=list(trace=3,reltol=1e-10,maxit=1000))
